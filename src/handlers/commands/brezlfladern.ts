...
export default (bot: TelegramBot) => {
	bot.onText(commandRegex('brezlfladern'), async msg => {
		...
		if (Date.now() - getLastRobbery(msg.chat.id, msg.from!.id) <= 7 * 24 * 60 * 60 * 1000) {
			await sendMessage(
				bot, msg,
				`De Behördn suchn no noch dia!
<b>Du musst a Wochn wardn bis du wieda Brezl fladern kannst!</b>`,
		...
		const reward = getRandomInt(0, 10);
		...
		// 6-7: Du warst erfolgreich, aber auf deiner Flucht wurdest du von einem Auto angefahren und hattest deshalb nur noch 7 Brezln.
		// 8: Du warst erfolgreich, aber auf deiner Flucht bist du hingefallen und hattest deshalb nur noch 8 Brezln.
		...
		let text: string = 'Du vasuchst auf am Oktobafest a boah Brezln zua fladern! &#x1F608;&#x1F968;\n\n';
		let comment: string;
		...
		switch (reward) {
			...
			case 6:
			case 7:
				text += `<b>Du warst eafolgreich, aba auf doana Flucht wurdest du vo am Auto ogfahrn und hattest deshoib grod no <code>${reward}</code> Brezln.</b>`;
				comment = `Wia hosd du des übalebt?! &#x1F631;&#x1F62C;`;
				removeButtonText = 'Aua scheise!';
				break;

			case 8:
				text += `<b>Du warst eafolgreich, aba auf doana Flucht bisd du higfoien und hattest deshoib grod no <code>8</code> Brezln.</b>`;
